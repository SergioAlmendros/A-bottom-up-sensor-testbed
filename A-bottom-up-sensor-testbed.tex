...
Este proyecto tiene como objetivo el despliegue de nodos de sensores por la red de guifi, una red abierta creada por sus usuarios, y la visualizacion de estos datos de una manera intuitiva que ayude a los usuarios finales a comprender el significado de éstos sin tener conocimientos previos.

Razon

Es importante para los ciudadanos tener una manera sencilla de obtener información del medioambiente de sus alrededores. De esta forma se podrá actuar en consecuencia si los sensores detectan situaciones perjudiciales, por ejemplo, que en un parque el sensor de gas detecte una mala calidad del aire.

Como

El despligue se hará sobre una red con nodos power over ethernet, por este motivo utilizamos un dispositivo que también lo es, por esto no nos tenemos que preocupar por temas de bateria. A partir de aquí, arduino Yun es la mejor opción para ser el componente principal del nodo sensor, y mediante una serie de sensores adjuntados a éste y la conexión a Internet que proporciona guifi, podemos comunicarnos con una plataforma de almacenamiento de datos de sensores, en éste caso, opencities. Finalmente, una aplicación android descargará los datos de los servidores de opencities y los mostrará en un mapa.
...
\label{Chapter3}
  ...
  \section{Python}
    A main part of the project is to upload the sensory data to opencities, and it has to be wrap into a GeoJSON message. The arduino sketch have a very low memory, so the best way to do this process is to use an external script stored in the SD card and make the sketch execute it. 
    
    One of the best option is a python script, because is really fast to code, it has all the libraries that we need, and, because this project could be update in the future, it is easy to read and understand.
  ...
  \section{Opencities}  
    Opencities\cite{opencities} is the opendata services that we have chosen, the strengths of opencities are:
    \begin{itemize}
    ...
    In opendata the sensor data will upload into a dataset. Every user can have one or more datasets, and to distinguish, the datasets have an unique ID, and every user have a unique API key. 

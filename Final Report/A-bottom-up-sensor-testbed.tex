...
\label{Chapter3}
  ...
  \section{Sensors}
    ...
    \subsection{LM35: Temperature}
      LM35 is a sensor with an output voltage proportional to the Centigrade temperature, the output pin goes directly to an analog pin in the arduino \ref{fig:LM35}.
...
\end{figure}
    ...
    \subsection{Light Dependent Resistor (LDR)}
      The LDR is a sensor which returns high voltage if there is more luminosity, and also it connects directly to an analog pin in the arduino, but with a resistor \ref{fig:LDR}.
...
\end{figure}
    ...
    \subsection{Emartee Mini Sound Sensor: Noise}
      This sensor is used to measured noise levels, which is similar to the LM35 in the connexion, the output pin goes directly to an analgo pin\ref{fig:MSS}.
...
\end{figure}
    ...
    \subsection{Aosong DHT22: Humidity}
      DHT22 is a humidity and temperature sensor, although we will only use the humidity measure. The output is digital, and to read it, we use an external library already developed \ref{fig:DHT22}.
...
\end{figure}
    ...
    \subsection{MQ135: Air Quality}
      This sensor is an optical dust sensor, and we will used to measured air quality. In this webpage\footnote{\url{http://arduino-info.wikispaces.com/Air-Gas-Sensors}} explain how to connected to the arduino, but the one that I bought has three pins joined in one, which make it easier \ref{fig:MQ135}.
...
\label{Chapter4}
  ...
  \section{Android app}
    ...
    \subsection{Code}
      ...
      The explanation of the code will be in the following flow chart \ref{}:

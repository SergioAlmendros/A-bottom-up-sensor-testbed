...
   \\
  L'objectiu principal és donar als ciutadans un camí per millorar la consciència ambiental, donant-lis la informació necessària per conèixer l'estat del seu entorn i que així puguin actuar en conseqüència.
...
   \\
  Arduino Yun és el component principal del node sensor. A través d'una sèrie de sensors connectats a ell i la connexió a Internet proporcionat per Guifi, el node es pot comunicar amb una plataforma d'emmagatzematge per a dades de sensors, en aquest cas, Opencities. Finalment, una aplicació per Android descàrrega les dades de sensors d'Opencities i els mostra en un mapa.
...
\label{Chapter2}
  ...
  \section{Introduction}
    Sensor networks started as a mechanism of defense developed by the United States Navy during the Cold War, with acoustic sensors they searched for Soviet submarines. This search continued at Universities, trying to make the sensors smaller, and with the possibility of real-time data \cite{chong2003sensor}.
    ...
    \begin{itemize}
    	\item {\bf Power:} A sensor node can obtain energy in several ways: batteries, solar energy, connecting to an already deployed network... The important thing is not to increase the size of the sensor, in our case, is power by an already deployed network.
  ...
  \section{Opendata services}
    The sensor boards are continuously collecting data, which has to be stored. The data can be stored locally or remotely. Collecting the data from the sensor board it is unpractical. For this reason an online storage service, opendata, is needed.    
...
\label{Chapter3}
  ...
  \section{Arduino-Raspberry PI comparison}
    ...
    For the Yun there is the possibility of a PoE module which it is not available yet, which will power the board. The figure \ref{fig:arduinoethernetpoe} and \ref{fig:poerpi} show the comparison between the size of an Arduino Ethernet (similar to the YUN) and the RPiB both powered by PoE. It is clearly visible that the Arduino is more compact.
...
\label{Chapter4}
  ...
  \section{Arduino Development}
    ...
    \subsection{Collect sensor data}
      ...
      The following libraries were needed to develop the code: FileIO, Process library, Bridge.
...
\end{figure}
	...
	\section{Repository}
		...
		In the figure \ref{fig:repository} it has been explained how the repository is distributed, each folder represents part of the process that has been done on this project.
		...
		\begin{itemize}
			\item {\bf Collect Data From Sensors:} Here has been stored the code to get the value of each sensor separately, in case someone wants to use only one of the sensors and does not want to read the full code.
			\item {\bf Collect Data From Sensors Nodes:} Using the logData files that have been created during the testbed and an octave script that has been developed to read this data and display it in graphs.
...
\label{Chapter5}
  ...
  Now, the process to configure the node is going to be explained, part of this process is from the website of Arduino\footnote{\url{http://arduino.cc/en/Guide/ArduinoYun}}:
  ...
  \subsection{Connection to the Internet}
    ...
    \subsubsection{Through Ethernet}
      This is the fastest way to provide of Internet connection, the Arduino behave the same way as a computer, automatically obtain an IP address.
  ...
  \subsection{Install necessary packets}
    The Arduino runs an Arduino script and a python script, for python some packets have to be installed.
  ...
  \subsection{Copy the scripts}
    ...
    Copy the python script ``main.py'' into the SD-Card. There are two ways to do this, putting the SDCard into a computer an saving the files in there, or copying the files into the Arduino through the network with the following command:
  ...
  \section{Actual Testbed}
    ...
    The location has to bee introduce manually into the python script and the unique ID into the Arduino sketch by entering into the Arduino by secure shell as it was explained earlier, go to the folder where the ``main.py'' is, and modified the following line by using ``nano'', a text editor:
    ...
    \subsection{Results}
      ...
      The temperature is clearly higher in Badalona, but the three locations are not far apart, so this value should not vary much. But it is important to notice that this is a low quality sensor so it is possible that it may have failed during the testbed, since the graphs of Montgat and Barcelona are similar between them, but very different in comparison to Badalona.
      
      In the case of light, the sensor node in Barcelona receives direct sunlight during most of the day, but the node in Montgat and Badalona do not. Consequently, it is normal that the Barcelona node had a much higher light value. It is important to consider that this node is located in an office, so is reasonable to think that the sudden drop from 100 lux to 10 or less may be because someone turn off the light of the office, and that the sunlight was gone a lot time ago.
      ...
      The Barcelona node collects noise values much higher than the other two nodes. The Badalona and Montgat nodes are in areas with punctual car traffic, while the Barcelona node is a busy area, causing more noise.

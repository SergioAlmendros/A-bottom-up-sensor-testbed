...
\label{Chapter3}
  ...
  \section{Sensors}
    ...
    To show how the sensors are connected I used the program fritzing\footnote{\url{http://fritzing.org/}}.
    ...
    \subsection{Light Dependent Resistor (LDR)}
      The LDR is a sensor which returns a value between 0 to 1024 depending on the light, and also it connects directly to an analog pin in the arduino, but with a resistor \ref{fig:LDR} as shown in the figure \ref{fig:LightSensor_bb}.
    ...
    \subsection{Emartee Mini Sound Sensor: Noise}
      This sensor is used to measured noise levels, which is similar to the LM35 in the connexion, the output pin goes directly to an analgo pin\ref{fig:MSS} as shown in the figure \ref{fig:NoiseSensor_bb}, but I did not find a figure for this sensor, so I put a microphone that has the same pins.
    ...
    \subsection{MQ135: Air Quality}
      This sensor is an optical dust sensor, and we will used to measured air quality. In this webpage\footnote{\url{http://arduino-info.wikispaces.com/Air-Gas-Sensors}} explain how to connect the sensor to the arduino, but the one that I bought has three pins joined in one, which make it easier \ref{fig:MQ135}, but this has no figure in fritzing, but I used a gas sensor that has the same output, and the resutl is in the figure \ref{fig:AirQualitySensor_bb}.
    ...
    \subsection{BreadBoard with all the sensors}
      ...
      This \ref{} is the final prototype of the arduino YUN and all the sensors connected to it.
      
      \begin{figure}[!htbp]
          \centering
              \includegraphics[scale=1]{./Figures/Fritzing/AllSensors_bb.png}
              \rule{35em}{0.5pt}
  ...
  \section{Guifi network and opencities}
    Guifi network\footnote{\url{https://www.guifi.net/}} is the network where the arduino's will be installed, and the one which will provide the access to the Internet and opencities.
    
    Opencities\footnote{\url{http://opencities.upf.edu/web/index.php/en/}} is the opendata services that we chose, the strengths of opencities are that give us free storage, a great and easy API to upload and download the data, and, also, the developers are in the UPF and problems will be solved more easily.
...
\label{Chapter4}
  ...
  \section{Android app}
    ...
    \subsection{Code}
      ...
      This application has the next classes, in the next figure \ref{fig:ClassDiagram} is the class diagram:
      \begin{itemize}
      ...
      \begin{figure}[htbp]
          \centering
              \includegraphics[scale=0.2]{./Figures/ClassDiagram.png}
              \rule{35em}{0.5pt}
